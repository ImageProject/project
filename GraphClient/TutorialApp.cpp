/*Это бесплатная программа предназначена для того, чтобы
 пользователи могли редактировать свои фото.
 Программа была создана в 2014 году 
 студентами группы 103 факультета ПМ-ПУ.
 Функционал программы таков: пользователь делает фото, 
 затем загружает её в наше приложение,
 а после этого может применить к ней множество разнообразных фильтров.
 Далее всё это можно сохранить себе на компьюетр и использовать.
 Сейчас приложение находится в стадии разработки.
 Но уже может похватастаться более 4 работающих фильтров,
 возможностью регистрации нескольких пользователей,
 а также непосредственно обработка фото
 и созранением их на компьюетр.
 Эту программу нельзя назвать полноценным 
 графическим редактором. Скорее это утилита для быстрой и 
 красивой обработки фотографий.
 Со своими обязанностями, однако, она вполне справляется. 
 Весь процесс происходит быстро и он интуитивно понятен 
 даже начинающим пользователям.
 Несомненное достоинство данного сервиса – обширная коллекция фильтров, 
 позволяющая создать эффект профессионального редактирования фотографии. 
 Наши эффекты позволяют превратить обычное фото в шедевр 
 или «вытянуть» не очень удачный кадр.
 */

/*В данном файле подключаются абсолютно все .h-файлы для полноценной работы приложения.
 И вообще это самый главный файл нашего приложения. Клиента точно.
 */
...
void TutorialApp::keyDown(KeyEvent k_event){
	...
	case KeyEvent::KEY_SPACE:
		...
		} else {// ÚÛÚ ·Û‰ÂÚ ÔÂÂÒ˚ÎÍ‡ ÓÚÔ‡‚Í‡ 
		}
	}         /// ÚÛÚ Ì‡‰Ó ËÒÔ‡‚ËÚ¸ ÌÂ·ÓÎÎ¸¯ÓÈ ·‡„, Ë·Ó ÓÚÔ‡‚ÎˇÚ¸Òˇ ‚ÒÂ ·Û‰ÂÚ ÚÓÎ¸ÍÓ ÔÓÒÎÂ Ì‡Ê‡ÚËˇ ÎÂ‚˚ı ÍÌÓÔÓÍ. ÕÛ Ë Î‡Ì
	}
	else {
...
//	if(next_form){
//		if (mVertexController.isTextBox(m_event.getPos()))          /// ˝ÚÓ ÓÔÂ‰ÂÎˇÂÚ, Ì‡Ê‡ÚËÂ Ì‡ ‚Â¯ËÌÛ ËÎË ÌÂÚ
//	/*		m_event.*/         // ÚÛÚ ‰ÓÎÊÌ‡ ·˚Ú¸ ÎÓ„ËÍ‡ ÔÂÂÏÂ˘ÂÌËˇ ˝ÎÂÏÂÌÚ‡
//	}
//}
